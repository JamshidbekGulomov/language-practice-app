...
**The homepage is organized into three tracks (`src/lib/tracks.ts`), not a flat module grid.** General English (`/general`) hosts the five non-exam-specific modules — Listening, Reading, Vocabulary, Writing, Translation — filtered from `MODULES` by slug. IELTS and CEFR each link to an exam hub page (`src/app/[exam]/page.tsx`) with three windows — Listening, Reading, Speaking — all scoped to that exam. `MODULES` (`src/lib/modules.ts`) still lists all six including Speaking — it's unchanged and still drives the admin dashboard/sidebar, since admin content management isn't reorganized by exam track. The old `/speaking` CEFR-or-IELTS chooser page still works as a direct link; it's just no longer linked from the homepage or footer.

**Exam-scoping for Listening and Reading** mirrors how Speaking already scopes itself, but simpler since neither has Speaking's part-nesting: `listening_clips` and `reading_passages` each carry a nullable `exam` column (`'ielts' | 'cefr'`, `null` = General English). The shared `Exam` type and `isExam()` guard live in `src/lib/exam.ts`; `src/lib/speaking/exams.ts` re-exports `SpeakingExam` as an alias of `Exam` for backward compatibility rather than defining its own. `listClips()`/`listPassages()` (`src/lib/listening/queries.ts` / `src/lib/reading/queries.ts`) take an optional `{ exam }` filter defaulting to `null`, so the existing `/listening` and `/reading` pages (General English) needed no changes. `src/app/[exam]/listening/page.tsx` and `src/app/[exam]/reading/page.tsx` are the exam-filtered list views; both link into the *same* `/listening/[slug]` and `/reading/[slug]` detail/game routes as General English, since slugs are globally unique regardless of exam tag — only the list pages differ. Admin: `NewClipForm` and the inline create form in `/admin/reading` each got a General/IELTS/CEFR `<select>`, and `ClipsTable`/`PassagesTable` show the resulting exam as a column; unlike Speaking, `exam` isn't editable after creation.
...
## Reading Exam and Listening Exam (computer-delivered style practice)

These are separate modules from General English's `reading_passages`/`listening_clips` — richer, exam-only content types that only exist under `/ielts` and `/cefr`, reachable via `/[exam]/reading` and `/[exam]/listening` (which now list these instead of the flat passage/clip lists General English still uses unchanged).

**Reading Exam** (`src/lib/reading-exam/`, admin at `/admin/reading-exam`): `reading_exam_tests` (exam, title, slug) each own exactly three `reading_exam_passages` (`passage_number` 1-3, unique per test). A passage's `paragraphs`, `glossary`, `question_groups`, and `paraphrase_pairs` are JSONB columns rather than normalized tables — a passage is authored and read as one atomic unit, and the shapes are nested/heterogeneous enough (three different question-group types) that join tables would add ceremony without benefit. `question_groups` is a discriminated union (`QuestionGroup` in `types.ts`) over exactly three types — `matching_headings`, `matching_features`, `summary_completion` — matching the practice format in the reference exam file the admin uploaded; a passage can include zero to three of them (toggled per-type in the admin passage editor, `PassageEditor`). Admin always types in the correct answers by hand (no answer-key extraction from PDFs) — `question_groups` items don't store a `question` number the admin edits directly; it's `startQuestion + index`, computed at save time, so reordering rows can't desync numbering.

`ExamPlayer` (`src/components/reading-exam/exam-player.tsx`) is the shared student-facing practice engine for both a single passage (`/reading-exam/[testSlug]/passage/[num]`) and the Full Exam (`/reading-exam/[testSlug]/full`, all three passages back-to-back, one 60-minute timer) — it takes `passages: ReadingExamPassage[]` (length 1 or 3) plus a scope label, so the two routes are thin wrappers around one component. Grading (`src/lib/reading-exam/grading.ts`) runs client-side per passage and sums; a result is persisted via `submitAttempt` (`src/app/reading-exam/actions.ts`, re-derives the user server-side rather than trusting a client-supplied id) to `reading_exam_attempts`, which is private per-student (RLS `auth.uid() = user_id`, no admin review queue — this is auto-graded, unlike Writing/Speaking). Glossary word-highlighting is automatic: `splitOnGlossary` regex-matches each glossary entry's word against paragraph text at render time, rather than requiring admin-authored inline markup.

After finishing, two follow-on activities reuse the passage's own data: **Word Practice** (`/reading-exam/[testSlug]/passage/[num]/words`) reuses the existing Vocabulary/Listening/Reading multiple-choice/fill-blank/matching game components against `reading_exam_words` (a plain word+meaning list, same shape as the other modules' `*_words` tables) — scores aren't persisted for this bonus activity, so the games are called with a no-op `onSubmitScore`. **Synonym Practice** (`.../synonyms`, `SynonymPractice` component) shows the passage alongside admin-authored `paraphrase_pairs` (`{keyword, phrase}`) and has the student type the passage's paraphrase for each question keyword — a distinct exercise from the main exam, not auto-derived from `question_groups`.

Scope deliberately excludes some of the reference file's polish to keep admin authoring tractable: no per-question "why is this the answer" explanations, no evidence-highlighting-in-passage-after-check, no drilling by question-type (practice is full-passage or full-exam only). PDF upload/auto-extraction was also deferred — admin authors passages directly in the structured editor.

**Listening Exam** (`src/lib/listening-exam/`, admin at `/admin/listening-exam`) is much simpler: `listening_exam_tests` (exam, title, slug) own 1-4 `listening_exam_sections` (`section_number`, `label`, `audio_path`, `transcript`), uploaded via the same signed-upload-URL flow and public `listening-audio` bucket as the General English Listening module. There's no auto-graded exam here — the student page (`/listening-exam/[testSlug]`, `ListeningExamPlayer`) is listen + read transcript + take notes, full stop. Notes autosave (debounced) to `listening_exam_notes` (one row per `(user_id, test_id)`, upserted via `onConflict`) and are visible to admin at `/admin/listening-exam/notes` — a plain reverse-chronological feed across all students, not a per-student review queue, since there's nothing to grade.
