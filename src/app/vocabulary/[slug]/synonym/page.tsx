...
export default async function VocabSynonymPage({
  ...
  const words = (await getWordsForCategory(category.id)).filter((w) => w.synonym);
  if (words.length < 2) {
    return (
      <p className="mx-auto max-w-md px-4 py-16 text-center text-slate-500">
        Not enough words with synonyms yet for this game.
      </p>
    );
  }

  const round = sample(words, Math.min(MATCHING_ROUND_SIZE, words.length));
  const pairs = round.map((w) => ({ id: w.id, left: w.english, right: w.synonym! }));
  const boundSubmit = submitScore.bind(null, category.id, slug, "synonym");
