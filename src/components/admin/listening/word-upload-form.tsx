...
import { bulkImportWords, type BulkListeningWordRow } from "@/app/admin/listening/actions";

const HEADER_ALIASES: Record<string, keyof BulkListeningWordRow> = {
  word: "word",
  english: "word",
  meaning: "meaning",
  translation: "meaning",
...
function parseRows(raw: Record<string, unknown>[]): BulkListeningWordRow[] {
  return raw.map((row) => {
    const mapped: BulkListeningWordRow = { word: "" };
...
export function ListeningWordUploadForm({ clipId }: { clipId: string }) {
    ...
    reader.onload = (e) => {
      try {
        ...
        startTransition(async () => {
          try {
            const result = await bulkImportWords(clipId, rows);
            setMessage(
