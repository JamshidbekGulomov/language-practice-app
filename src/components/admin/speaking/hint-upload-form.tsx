...
import { bulkImportHints, type BulkSpeakingHintRow } from "@/app/admin/speaking/actions";

const HEADER_ALIASES: Record<string, keyof BulkSpeakingHintRow> = {
  word: "word",
  english: "word",
  meaning: "meaning",
  translation: "meaning",
...
function parseRows(raw: Record<string, unknown>[]): BulkSpeakingHintRow[] {
  return raw.map((row) => {
    const mapped: BulkSpeakingHintRow = { word: "" };
...
export function SpeakingHintUploadForm({ topicId }: { topicId: string }) {
    ...
    reader.onload = (e) => {
      try {
        ...
        startTransition(async () => {
          try {
            const result = await bulkImportHints(topicId, rows);
            setMessage(
